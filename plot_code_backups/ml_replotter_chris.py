...
f2,ax2 = plt.subplots(1,1,figsize=(4.5,3.3),dpi=100)
...
for i in range(0,len(ratio_vec)): 
    ...
    if i == 0:
        ax2.plot(temp_indexes,full_fl_ratios[i][0],\
            label='Standard '+ r'$\tau_{z}^{\mathsf{G}} = 1$',\
            color='darkgoldenrod',linestyle='solid',linewidth=lwd)#,marker='x')
        ax2.plot(temp_indexes,full_pfl_ratios[i][0],\
            label='Meta-learning '+ r'$\tau_{z}^{\mathsf{G}} = 1$',\
    ...
    #         color='black',linestyle='dotted',linewidth=lwd)#,marker='x')
        ...
        ax2.plot(temp_indexes,full_fl_ratios[i][0],\
            label='Standard '+ r'$\tau_{z}^{\mathsf{G}} = 8$',\
            color='darkgoldenrod',linestyle='dashdot',linewidth=lwd)#,marker='x')
        ax2.plot(temp_indexes,full_pfl_ratios[i][0],\
            label='Meta-learning'+ r'$\tau_{z}^{\mathsf{G}} = 8$',\
...
# ax2.set_xticklabels(txlab,fontsize=12)
# ax2.set_yticklabels(tylab,fontsize=12)
ax2.tick_params(axis='both',which='major',labelsize=13.5)

ax2.set_ylabel('Classification Accuracy (%)',fontsize=15)
ax2.set_xlabel('Local Iteration',fontsize=15)
ax2.grid(True)
...
#(x, y, width, height)
leg1 = ax2.legend(h[0:2],l[0:2],bbox_to_anchor=(-0.22,1.08,1.32,0.2),\
                        mode='expand',fontsize=14,**kw2)
leg2 = ax2.legend(h[2:],l[2:],bbox_to_anchor=(-0.22,0.96,1.32,0.2),\
...
plt.savefig(cwd+'/ml_plots/mild_'+data_source+'_ovr_swarms_chris.pdf',bbox_inches='tight')
