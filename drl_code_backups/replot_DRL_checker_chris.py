...
f1,ax1 = plt.subplots(1,2,figsize=(7.5,4))
...
for ep_start in [0.7]:#[0.6,0.8]:
    for gamma in [0.6]:#,0.7]:#[0.7,0.8]:
        ...
        if ep_start == 0.6:
            if gamma == 0.6:
                ax1[0].plot(data_b2,label=r'DRL'\
                    ,linestyle='dotted',\
                        linewidth=lwd,color = 'darkgreen') #+ r' $\epsilon$ = ' + str(ep_start)
            elif gamma == 0.7:
                ax1[0].plot(data_b2,label=r'DRL' \
            ...
            if gamma == 0.6:
                ...
                ax1[0].plot(data_b2,label=r'DRL: $\gamma$ = '+str(gamma) \
                    ,linestyle='dashed', \
                        color = 'magenta',linewidth=lwd) #+ r' $\epsilon$ = ' + str(ep_start)
            else:
                ax1[0].plot(data_b2,label=r'DRL: $\gamma$ = '+str(gamma) \
...
ax1[0].set_title('(a)',fontsize=20,y=-0.42) # Battery Over Time',fontsize=15,y=-0.24)
ax1[0].grid(True)
ax1[0].set_xlabel('Epoch',fontsize=20)
ax1[0].set_ylabel('Avg Battery Level (kJ)',fontsize=20)
...
for ep_start in [0.7]:#[0.6,0.8]:
    for gamma in [0.6]:#,0.7]:#[0.7,0.8]:
        ...
        if ep_start == 0.6:
            if gamma == 0.6:
                ax1[1].plot(data_ml2,label=r'DRL'\
                    ,linestyle='dotted',\
                        linewidth=lwd,color = 'darkgreen') #+ r' $\epsilon$ = ' + str(ep_start)
            elif gamma == 0.7:
                ax1[1].plot(data_ml2,label=r'DRL' \
            ...
            if gamma == 0.6:
                ax1[1].plot(data_ml2,label=r'DRL: $\gamma$ =' + str(gamma)\
                    ,linestyle='dotted',\
                        linewidth=lwd,color = 'darkgreen') #+ r' $\epsilon$ = ' + str(ep_start)
            elif gamma == 0.7:
                ax1[1].plot(data_ml2,label=r'DRL: $\gamma$ = '+str(gamma) \
...
# ax1[1].set_title('b)',fontsize=20,y=-0.32) # Battery Over Time',fontsize=15,y=-0.24)
ax1[1].set_title('(b)',fontsize=20,y=-0.42) # Battery Over Time',fontsize=15,y=-0.24)
ax1[1].grid(True)
ax1[1].set_xlabel('Epoch',fontsize=20)
ax1[1].set_ylabel('Learning Objective',fontsize=20)
...
for ep_start in [0.7]:#[0.6,0.8]:
    for gamma in [0.7]:#[0.7,0.8]:
        for g_ind,greed_style in enumerate([1,2]): #[0,1,2]):
            if greed_style != 2:
            ...
            if greed_style == 0:
                ...
                ax1[0].plot(g_bat2,label='S.H.',color='saddlebrown',\
                    linewidth = lwd) #Sequential Heuristic
                    
                ax1[1].plot(g_ml2,label='S.H.',color='saddlebrown',\
                    linewidth = lwd)
                ...
                ax1[0].plot(g_bat2,label='G.M.D.',color='slategrey',\
                    linewidth = lwd)
                    
                ax1[1].plot(g_ml2,label='G.M.D.',color='slategrey',\
                    linewidth = lwd)                    
            else:
                # ax1[0].plot(g_reward2,label='T.M.D.',color='darkorange',\
                    # linewidth = lwd) # T.M.D. - threshold minimum distance

                ax1[0].plot(g_bat2,label='T.M.D.',color='darkorange',\
                    linewidth = lwd) # T.M.D.
                
                ax1[1].plot(g_ml2,label='T.M.D.',color='darkorange',\
                    linewidth = lwd)                                     
        # plt.savefig(cwd+'/drl_plots/freq_no_recharge.pdf',dpi=1000, bbox_inches='tight')
...
leg1 = ax1[0].legend(h[:],l[:],bbox_to_anchor=(-0.15,0.95,2.5,0.2),\
                mode='expand',fontsize=18,**kw)
...
ax1[0].set_yticklabels(['25','30','35','40','45'])

ax1[0].set_xticks([0,10000,20000,30000])
ax1[1].set_xticks([0,10000,20000,30000])
f1.tight_layout()
...
f1.savefig(cwd+'/drl_plots/chris2_drl_ovr_comp.pdf',dpi=1000, bbox_inches='tight')
